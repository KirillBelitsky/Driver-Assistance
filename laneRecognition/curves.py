...
class Curves:
    ...
    def __store_details(self, binary):
    ...
    def __pixel_locations(self, indices):
        return self.all_pixels_x[indices], self.all_pixels_y[indices]

    def __define_direction(self, xls, xrs):
        lx = np.trunc(xls)
        rx = np.trunc(xrs)
    ...
    def __plot(self, t=4):
        ...
        self.__define_direction(xls, xrs)
    ...
    def __get_real_curvature(self, xs, ys):
        return np.polyfit(ys * self.ky, xs * self.kx, 2)

    def __radius_of_curvature(self, y, f):
        return ((1 + (2 * f[0] * y + f[1]) ** 2) ** (1.5)) / np.absolute(2 * f[0])

    def __update_vehicle_position(self):
        y = self.h
        mid = self.w / 2
        kl, kr = self.left_fit_curve_pix, self.right_fit_curve_pix
    ...
    def fit(self, binary):
        ...
        self.__store_details(binary)
        mid_leftx, mid_rightx = self.__start(binary)
        ...
        try:
            ...
            for w in range(self.n):
                y[0], y[1] = self.__next_y(w)
                x[0], x[1] = self.__next_x(mid_leftx)
                x[2], x[3] = self.__next_x(mid_rightx)

                self.__draw_boundaries((x[0], y[0]), (x[1], y[1]), (255, 0, 0))
                self.__draw_boundaries((x[2], y[0]), (x[3], y[1]), (0, 255, 0))

                curr_left_pixels_indices = self.__indices_within_boundary(y[0], y[1], x[0], x[1])
                curr_right_pixels_indices = self.__indices_within_boundary(y[0], y[1], x[2], x[3])

                left_pixels_indices.append(curr_left_pixels_indices)
                right_pixels_indices.append(curr_right_pixels_indices)

                mid_leftx = self.__next_midx(mid_leftx, curr_left_pixels_indices)
                mid_rightx = self.__next_midx(mid_rightx, curr_right_pixels_indices)

            self.left_pixels_indices = np.concatenate(left_pixels_indices)
            self.right_pixels_indices = np.concatenate(right_pixels_indices)

            self.left_pixels_x, self.left_pixels_y = self.__pixel_locations(self.left_pixels_indices)
            self.right_pixels_x, self.right_pixels_y = self.__pixel_locations(self.right_pixels_indices)

            self.left_fit_curve_f = self.__get_real_curvature(self.left_pixels_x, self.left_pixels_y)
            self.right_fit_curve_f = self.__get_real_curvature(self.right_pixels_x, self.right_pixels_y)

            self.left_radius = self.__radius_of_curvature(self.h * self.ky, self.left_fit_curve_f)
            self.right_radius = self.__radius_of_curvature(self.h * self.ky, self.right_fit_curve_f)

            self.__plot()
            self.__update_vehicle_position()
        except Exception:
            print('Error')
