...
class Dataset(object):
    ...
    def __init__(self, FLAGS, is_training, dataset_type="converted_coco"):
        ...
        self.classes = Util.read_class_names(cfg.YOLO.CLASSES)
    ...
    def load_annotations(self):
        with open(self.annot_path, "r") as f:
            txt = f.readlines()
            if self.train_settings['dataset_type'] == "converted_coco":
                annotations = [
                    line.strip()
                    for line in txt
                    if len(line.strip().split()[1:]) != 0
                ]
            elif self.train_settings['dataset_type'] == "yolo":
                annotations = []
                for line in txt:
    ...
    def __next__(self):
        with tf.device("/cpu:0"):
            ...
            self.train_output_sizes = self.train_input_size // self.train_settings['strides']
            ...
            for size in self.train_output_sizes:
                label_bbox = np.zeros((self.batch_size, size, size, self.train_settings['anchor_per_scale'],
                                       5 + self.num_classes),
                                       dtype=np.float32)
    ...
    def parse_annotation(self, annotation):
        ...
        image, bboxes = Util.image_preprocess(
            np.copy(image),
    ...
    def preprocess_true_boxes(self, bboxes, num_anchors=3):
        label = [
            np.zeros(
                (
                    ...
                    self.train_output_sizes[i],
                    self.train_settings['anchor_per_scale'],
                    5 + self.num_classes,
                )
            )
        ...
        for bbox in bboxes:
            ...
            bbox_xywh_scaled = (
                    1.0 * bbox_xywh[np.newaxis, :] / self.train_settings['strides'][:, np.newaxis]
            )

            iou = []
            exist_positive = False
            for i in range(num_anchors):
                anchors_xywh = np.zeros((self.train_settings['anchor_per_scale'], 4))
                anchors_xywh[:, 0:2] = (
                        np.floor(bbox_xywh_scaled[i, 0:2]).astype(np.int32) + 0.5
                )
                anchors_xywh[:, 2:4] = self.train_settings['anchors'][i]

                iou_scale = Util.bbox_iou(
            ...
            if not exist_positive:
                ...
                best_detect = int(best_anchor_ind / self.train_settings['anchor_per_scale'])
                best_anchor = int(best_anchor_ind % self.train_settings['anchor_per_scale'])
                xind, yind = np.floor(
